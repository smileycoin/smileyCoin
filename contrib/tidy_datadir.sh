...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Auroracoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Auroracoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Auroracoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Auroracoin 0.7 datadir."
    ;;
  3)
    echo "Detected Auroracoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Auroracoin 0.8 datadir."
    ;;
esac

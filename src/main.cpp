...
#if defined(NDEBUG)
# error "Tutorcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tutorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tutorcoin Signed Message:\n";
...
{
    // Tutorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//TODO: Edit blockvalue.
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Tutorcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

//TODO: Edit these for timespan and difficulty spacing.
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Tutorcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Tutorcoin: 2.5 minutes
...
//TODO: adjust re-target time for difficulty.
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tutorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tutorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Tutorcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        //TODO: Genesis block stuffs.
        //          Timestamp
        //          Epoch value
        //          Merkel hash
        //          Network bits (maybe experiment with that?)
        //          Nonce and genesis hash
        // Genesis block
        ...
        // Loop to generate new genesis hash and nonce if it doesn't match.
        // TODO: change to false when hash has been generated
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Tutorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TutorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TutorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TutorcoinMiner(CWallet *pwallet)
{
    printf("TutorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tutorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TutorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TutorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TutorcoinMiner, pwallet));
}

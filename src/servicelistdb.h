...
typedef std::pair<std::string, std::string> CServiceInfo;
...
private:
    ...
    std::string ServiceName(const mapServiceScriptPubKeys::iterator &it) const { return it -> second.second; }
    std::string ServiceType(const mapServiceScriptPubKeys::iterator &it) const { return it -> second.first; }
    std::string ServiceName(const mapServiceScriptPubKeys::const_iterator &it) const { return it -> second.second; }
...
public:
    ...
    bool GetServiceAddresses(std::multiset<std::pair<CScript, std::pair<std::string, std::string>>> &retset) const;

    bool SetForked(const bool &fFork);
    bool IsForked(){return fForked;}

    bool UpdateServiceInfo(const std::map<CScript, std::pair<std::string, std::string> > &map);
